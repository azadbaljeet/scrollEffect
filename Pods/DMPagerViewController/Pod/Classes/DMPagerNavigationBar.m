...
+ (DMPagerNavigationBarItem *) newItemWithText{
    NSDictionary *textAttributes = @{ NSFontAttributeName : [UIFont systemFontOfSize:14.0f],NSForegroundColorAttributeName : [UIColor blackColor]};
    DMPagerNavigationBarItem *item = [[DMPagerNavigationBarItem alloc] init];
    item.title = [[NSAttributedString alloc] initWithString:@"hello MAkarshankranti" attributes:textAttributes];
   // [item layoutSubviews];
    return item;
//	DMPagerNavigationBarItem *item = [[DMPagerNavigationBarItem alloc] init];
//	item.title = [[NSAttributedString alloc] initWithString:@"HELLO"];
//	//item.icon = aIcon;
//	[item layoutSubviews];
//	return item;
}

+ (DMPagerNavigationBarItem *) newItemWithTextSmall:(NSString *) myTitle{
    DMPagerNavigationBarItem  *itm  =   [[DMPagerNavigationBarItem alloc] init];
    itm.myTitle   =   @"TODAY";
    [itm layoutSubviews];
    return itm;
...
- (void)layoutSubviews {
    ...
    CGSize titleSize = [_title boundingRectWithSize:CGSizeMake(navigationBarSize.width, navigationBarSize.height)options:NSStringDrawingTruncatesLastVisibleLine context:NULL].size;
    
    CGRect titleRect = CGRectMake(((CGRectGetWidth(self.bounds)-titleSize.width)/2.0f),20,titleSize.width,titleSize.height);
    
    self.frame = CGRectMake(self.frame.origin.x,self.frame.origin.y,titleRect.size.width,self.frame.size.height);
...
- (void)setItems:(NSArray *)items {
	...
	for (NSUInteger idx = 0; idx < _items.count; ++idx) {
        ...
        cItem.backgroundColor   =   [UIColor greenColor];
